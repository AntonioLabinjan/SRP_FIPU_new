...
def transform_person_dim(mysql_person_df, csv_person_df=None, party_dim_df=None):
    ...
    mysql_df = (
        ...
        .dropna(subset=["person_id", "first_name", "last_name", "birth_year", "title", "party_id"])
        .dropDuplicates(["person_id", "first_name", "last_name", "birth_year", "title", "party_id"])
    )
    ...
    if csv_person_df:
        ...
        combined_df = mysql_df.unionByName(csv_df).dropDuplicates(["person_id", "first_name", "last_name", "birth_year", "title", "party_id"])
    else:
    ...
    window = Window.orderBy("person_id", "first_name", "last_name")
    final_df = combined_df.withColumn("person_tk", row_number().over(window)) \
                          .select("person_tk", "person_id", "first_name", "last_name", "birth_year", "title", "party_id")
