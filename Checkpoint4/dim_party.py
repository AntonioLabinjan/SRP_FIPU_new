...
max_id_result = conn.execute(table.select().with_only_columns(table.c.party_id).order_by(table.c.party_id.desc()).limit(1)).fetchone()
...
for idx, value in enumerate(unique_values, start=max_id + 1):
    try:
        ...
        ins = insert(table).values({table.c.party_id: idx, TABLE_COLUMN: value})  # Zamijeni 'country_id' s pravim nazivom ID stupca
        conn.execute(ins)
