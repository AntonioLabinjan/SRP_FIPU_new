...
def transform_elections_fact(
   ...
   # dim_party_df,
    ...
    cleaned_csv = (
        csv_df
        .withColumn("territoryName", initcap(trim(col("country_name"))))
    ...
    enriched_df = (
        cleaned_csv.alias("c")
        .join(dim_country_df.alias("co"), col("c.territoryName") == col("c.country_name"), "left")
       # .join(dim_party_df.alias("p"), col("c.name") == col("p.name"), "left")
